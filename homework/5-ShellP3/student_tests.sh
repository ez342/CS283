...
EOF
    ...
    stripped_output=$(echo "$output" | sed 's/dsh3> //g')
...
EOF
    ...
    expected_output="forkerrorinexecvp:Nosuchfileordirectorydsh3>dsh3>dsh3>cmdloopreturned0"
...
EOF
	# Strip the prompt part (dsh3>) from the output
	stripped_output=$(echo "$output" | sed 's/dsh3> //g' | sed 's/cmd loop returned 0//g' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
...
EOF
	...
	stripped_output=$(echo "$output" | sed 's/dsh3> //g' | sed 's/cmd loop returned 0//g')
