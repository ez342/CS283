...
int get_student(int fd, int id, student_t *s)
{
	...
	char zeroBytes[STUDENT_RECORD_SIZE];
    memset(zeroBytes, 0, STUDENT_RECORD_SIZE);
    if (memcmp(s, zeroBytes, STUDENT_RECORD_SIZE) == 0) {
        return SRCH_NOT_FOUND;
    }
 	return NO_ERROR;
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    int checkExists = get_student(fd, id, &tempStudent);

    if (checkExists == NO_ERROR) {
    	printf(M_ERR_DB_ADD_DUP, id);
    	return ERR_DB_OP;
	} else if (checkExists == ERR_DB_FILE) {
	...
	ssize_t writeBytes = write(fd, &newStudent, STUDENT_RECORD_SIZE);
	if (writeBytes == -1 || writeBytes != STUDENT_RECORD_SIZE) {
...
int del_student(int fd, int id)
{
    ...
    if (checkExists == ERR_DB_FILE) {
		printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
    } else if (checkExists == SRCH_NOT_FOUND) {
	...
	ssize_t writeBytes = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
	if (writeBytes == -1 || writeBytes != STUDENT_RECORD_SIZE) {
		printf(M_ERR_DB_WRITE);
		return ERR_DB_FILE;
	}
...
{ 
	int recordNum = MIN_STD_ID;
	int recordsFound = 0;
	off_t offset = (off_t)MIN_STD_ID * STUDENT_RECORD_SIZE;
	...
	while (recordNum != MAX_STD_ID) {
		ssize_t bytesRead = read(fd, &tempStudent, STUDENT_RECORD_SIZE);
		if (bytesRead < 0) {
			printf(M_ERR_DB_READ);
			return ERR_DB_FILE;
		}

		if (memcmp(zeroBytes, &tempStudent, STUDENT_RECORD_SIZE) != 0) {
			recordsFound++;
		}
		recordNum++;
		offset = (off_t)recordNum * STUDENT_RECORD_SIZE; 
	...
	if (recordsFound == 0) {
    	...
    	printf(M_DB_RECORD_CNT, recordsFound);
    	return recordsFound;
	}
}
...
int print_db(int fd)
{
    ...
    while (recordNum != MAX_STD_ID) {
        ...
        if (memcmp(zeroBytes, &tempStudent, STUDENT_RECORD_SIZE) != 0) {
        	//saving data and printing once memory is not 0 bytes 
			recordsFound++;
			if (recordsFound == 1) {
				printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME", "GPA");				
			}
			gpa = (tempStudent.gpa / 100.0);
			printf(STUDENT_PRINT_FMT_STRING, tempStudent.id, tempStudent.fname, tempStudent.lname, gpa);	
